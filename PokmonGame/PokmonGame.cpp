...
void PKlistshow()
{
	...
	outtextxy(92, 130, hp);
	/*其余五只*/
	for (int i = 0; i < player.theMON.size(); i++, y1 += 60, y2 += 60) {
		if (i == player.useNo) {
			y1 -= 60;
			y2 -= 60;
			continue;
...
void BagUse()
{
	...
	if (codei < player.item.size()) {
		...
		if (potion.count > 0) {
			...
			int t1 = 0, t2 = 0;
			flag = potion.count;
			player.theMON[player.useNo].use(potion);
...
void battleshow(const Npc& emy)  //绘制战斗场景
{
	...
	Settxt(BLACK);
	//主角
	putimage(10, 120, &statebk[player.theMON[player.useNo].property]);//状态窗口		
